...
public class SubscribeService {
    ...
    public ClientCallback<Responses<Subcribes>> subscribe(final Integer userId, final Integer placeId, final ClientCallback<Responses<Subcribes>> clientCallback,
                                                          final boolean flag){ //true подписаться, false отписаться
        final UIThread<Responses<Subcribes>> responsesUIThread= new UIThread<>();
        executorService.execute(new Runnable() {
            @Override
            public void run() {
                try{
                    Response<Responses<Subcribes>> response;
